...
from ui.add_task_frame import AddNewTaskFrame
from ui.task_component import TaskUnit
from utils.storage import Storage, Task
...
class TodoFrame(customtkinter.CTkFrame):
    def __init__(self, rf, **kwargs):
        ...
        self.change_layout = customtkinter.CTkOptionMenu(
            master=self,
            height=25,
            values=list(self.layout_options.keys()),  # Use keys for display in the option menu
            command=lambda x: self.grid_frame.update_layout(column_count=self.layout_options[x])
        )
